...
def subtract_bias(inputs, bias, outputs=None, ovs_function='spline3',
    ...
    outputs: DataFileList or DataFile, optional.
        ...
        files, prefixed with 'rg' as in the Gemini IRAF package.
    ...
    # Avoid writing out where GMOS config files are kept N times:
    gmosdata = 'gmos$data/'
    ...
    # Enable both VAR & DQ if either is specified, since this task can't
    ...
    result = run_task('gemini.gmos.gfreduce', inputs={'inimages' : inputs,
        ...
        fl_vardq=vardq, fl_addmdf=True, fl_over=True, fl_trim=True,
        ...
        mdffile='default', mdfdir=gmosdata, key_biassec='BIASSEC',
        key_datasec='DATASEC', bpmfile=gmosdata+'chipgaps.dat', grow=1.5,
        ...
        gratingdb=gmosdata+'GMOSgratings.dat',
        filterdb=gmosdata+'GMOSfilters.dat', xoffset=iraf.INDEF,
        expr='default', sepslits=False, w1=iraf.INDEF, w2=iraf.INDEF,
        dw=iraf.INDEF, nw=iraf.INDEF, observatory='default',
        sci_ext=config['data_name'], var_ext=config['uncertainty_name'],
        dq_ext=config['flags_name'], verbose=verbose)
